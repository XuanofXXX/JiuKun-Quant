...
class LSTMTradingStrategy(BaseStrategy):
    def __init__(self, model_path="trained_lstm_stock_model(1).pth", lookback_period=20, sequence_length=10):
        super().__init__()
    ...
    def calculate_factors(df, window_short=5, window_medium=10, window_long=30):
    ...
    async def update_market_data(self, api, token_ub):
        try:
            tasks = [api.sendGetAllLimitOrderBooks(token_ub) for _ in range(4)]
    ...
    def calculate_order_size(self, instrument, user_info, price, max_position=2000, t=0, pred_price_limit=None):
        ...
        # 根据当前时间调整订单大小
        time_factor = min(1, max(0.1, t / 3000))  # 假设交易日为3000秒，随时间增加而增大
        ...
        try:
            ...
            for side, candidates in [("buy", buy_candidates), ("sell", sell_candidates)]:
                for _, row in candidates.iterrows():
                    ...
                    if temp_t != t:
                        logger.warning(f"current tick {t} does not exist, using {temp_t}")
